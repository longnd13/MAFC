...
(function (factory) {
    ...
    tModalMain = '<div id="' + MODAL_ID + '" class="file-zoom-dialog modal fade" tabindex="-1" aria-labelledby="' +
        MODAL_ID + 'Label"></div>';
    tModal = '<div class="modal-dialog modal-md" role="document">\n' +
    ...
    FileInput.prototype = {
        ...
        _addError: function (msg) {
            var self = this, $error = self.$errorContainer,$zone = self.$container.find('.file-drop-zone'), title = self.dropZoneTitle;
            if (msg && $error.length) {
        ...
        _change: function (e) {
            ...
            if (isEmpty(tfiles) || tfiles.length === 0) {
                var $zone = self.$container.find('.file-drop-zone'), title = self.dropZoneTitle;
                if (self.getFileStack().length == 0) {
                    $zone.prepend('<div class="' + self.dropZoneTitleClass + '">' + title + '</div>');
        ...
        clearStack: function () {
            ...
            self.filestack = [];
            self.filenames = [];
            var globalFileArr = self.$element.attr('id').replace('-', '').replace('_', '') + 'FileList';
            delete window.globalFileArr;
            return self.$element;
        },
...
;
        ...
        getFileStack: function (skipNull) {
            var self = this;
            var result = self.filestack.filter(function (n) {
                return (skipNull ? n !== undefined : n !== undefined && n !== null);
            });
            var globalFileArr = self.$element.attr('id').replace('-','').replace('_','') + 'FileList';

            window[globalFileArr] = result;
            return result;
        },
    ...
    $.fn.fileinputLocales.en = {
        ...
        msgZoomModalHeading: '',// custom by Otos
